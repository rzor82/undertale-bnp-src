...
switch argument1
{
    ...
    case 26:
        ...
        else
        {
            ...
            if (global.weapon == 47)
                global.msg[1] = "* Somehow^1, you recovered 4 HP!/%"
            else
                global.msg[1] = scr_gettext("item_use_26b")
            if (global.weapon == 47)
            {
                script_execute(scr_recoitem, 0)
                instance_create(0, 0, obj_foodsound)
            }
            script_execute(scr_writetext, 0, "x", 0, 0)
            script_execute(scr_itemshift, argument0, 0)
    ...
    case 64:
        ...
        break
    case 65:
        if (global.dogscepter == 1)
        {
            global.msg[0] = "* Dog Scepter deactivated./%%"
            global.debug = false
        }
        else
        {
            global.msg[0] = "* Dog Scepter activated!/%%"
            global.debug = true
        }
        global.dogscepter = (!global.dogscepter)
        snd_play(snd_item)
        script_execute(scr_writetext, 0, "x", 0, 0)
        break
    case 201:
