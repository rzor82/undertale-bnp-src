...
global.pca = 0
global.magicuse = 0
if (!variable_global_exists("origws"))
    global.origws = surface_get_width(application_surface)
if (!variable_global_exists("orighs"))
    global.orighs = surface_get_height(application_surface)
if (!variable_global_exists("draw_debug"))
    global.draw_debug = 1
if (!variable_global_exists("skip_to_menu"))
...
if (!variable_global_exists("tempsprite"))
{
    for (i = 0; i < 3; i++)
    {
...
global.batmusicintro = -1
global.batmusic = -1
global.typer = 5
global.msc = 0
global.hardmode = 0
global.face_any_xoff = -30
global.face_any_yoff = 30
global.dogscepter = 0
global.paused = false
global.allowpause = 1
global.partycount = 1
global.ambush = 0
if (!variable_global_exists("allowinternet"))
    global.allowinternet = 0
instance_activate_object(obj_dropped_item)
with (obj_dropped_item)
    instance_destroy()
global.uniqueitems = [3, 4, 11, 12, 13, 23, 24, 25, 27, 36, 46, 47, 48, 49, 50, 51, 52, 53, 56, 57, 60, 61, 63]
SCR_BORDERSETUP()
