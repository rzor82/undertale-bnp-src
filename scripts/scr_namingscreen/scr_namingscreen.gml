...
if (naming == 5)
{
    ...
    if (alerm > 179)
    {
        ...
        if (truereset > 0)
        {
            ...
            var Won = (ini_read_real("General", "Won", 0) || ini_read_real("General", "BW", 0))
            var CP = (ini_read_real("General", "CP", 0) || ini_read_real("General", "BP", 0))
            var CH = (ini_read_real("General", "CH", 0) || ini_read_real("General", "BH", 0))
            ossafe_ini_close()
...
if (naming == 3)
{
    ...
    if (ini_section_exists("General") && hasname == 1)
    {
        ...
        scr_drawtext_outline(namepos, 62, string_hash_to_newline(name))
        scr_drawtext_outline(lvpos, 62, string_hash_to_newline(lvtext))
        scr_drawtext_outline((timepos - timesize), 62, string_hash_to_newline(timetext))
        if (global.language == "ja" || string_width(roomname) > (room_width - (namepos * 2)))
            scr_drawtext_outline(((room_width - string_width(roomname)) / 2), 80, roomname)
        else
            scr_drawtext_outline(namepos, 80, string_hash_to_newline(roomname))
        if (selected3 == 0)
            draw_set_color(c_yellow)
        var continue_text = scr_gettext("load_menu_continue")
        scr_drawtext_outline(continue_x, 105, string_hash_to_newline(continue_text))
        draw_set_color(c_white)
        draw_set_color(c_white)
        if (selected3 == 2)
            draw_set_color(c_yellow)
        var settings_text = scr_gettext("settings_name")
        scr_drawtext_outline(((room_width - string_width(settings_text)) / 2), 125, settings_text)
        draw_set_color(c_white)
        if (selected3 == 1)
            draw_set_color(c_yellow)
        if (truereset == 0)
        ...
        scr_drawtext_outline(reset_x, 105, string_hash_to_newline(reset_text))
        if (keyboard_check_pressed(global.keybind[2]) || keyboard_check_pressed(global.keybind[0]))
        {
            if (selected3 == 0)
                selected3 = 1
        ...
        if keyboard_check_pressed(global.keybind[3])
        {
            if (selected3 == 0 || selected3 == 1)
                selected3 = 2
            keyboard_clear(global.keybind[3])
        }
        if keyboard_check_pressed(global.keybind[1])
        {
            if (selected3 == 2)
