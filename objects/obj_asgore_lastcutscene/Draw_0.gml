...
if (asgore == 1)
{
    ...
    if (remove == 1)
    {
        ry += 2
        if (ry < 49)
            draw_sprite_part_ext(spr_asgore_kneel, 0, 0, (0 + (ry * 2)), 183, (97 - (ry * 2)), (asgorex - 2), (asgorey + (ry * 4)), 2, 2, c_white, 1)
        if (ry < 27)
            draw_sprite_part_ext(spr_asgore_lastface, global.faceemotion, 0, (0 + (ry * 2)), 53, (54 - (ry * 2)), (asgorex + 132), ((asgorey - 5) + (ry * 4)), 2, 2, c_white, 1)
    }
}
if (con == 2)
{
...
if (con == 30)
{
    ddd = instance_create((x - 2), (y - 2), obj_vaporized_new)
    with (ddd)
        scr_newvapordata(34)
    remove = 1
...
if (flowey == 3)
{
    ...
    with (outline)
    {
        merge = floor(other.fimg2) != floor(prev)
        image_index = 0
        visible = true
        step = 0
    }
...
if (con == 106)
{
    var white_blocker = scr_marker(-10, -10, spr_pixwht)
    white_blocker.image_xscale = 900
    white_blocker.image_yscale = 900
    with (obj_expander)
    {
        white_blocker.x = (-diffx)
        white_blocker.y = (-diffy)
        white_blocker.image_xscale = window_get_width()
        white_blocker.image_yscale = window_get_height()
    }
    global.filechoice = 8
