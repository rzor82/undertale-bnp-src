...
if (con == 12 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    if (skydiving == 7)
        global.msg[1] = "I HAVE TO GO&SKYDIVING^1, OUT&OF THE WINDOW!!/"
    instance_create(0, 0, obj_dialoguer)
    con = 13
    with (papyrus)
...
if (con == 60 && instance_exists(OBJ_WRITER) == 0 && global.interact == 0)
{
    ...
    global.phasing = 1
    with (obj_mainchara)
        scr_getideal(chair.x, (chair.y - 16))
    obj_mainchara.cutscene = true
    obj_mainchara.depth = (chair.depth - 100000000000000)
    obj_mainchara.fun = true
    obj_mainchara.sprite_index = spr_mainchara_undynesit
...
if (con == 81 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    obj_mainchara.sprite_index = spr_maincharau
    con = 82
...
if (con == 85 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    with (obj_mainchara)
        scr_getideal(chair.x, (chair.y - 16))
    obj_mainchara.cutscene = true
    obj_mainchara.depth = (table.depth - 1)
    obj_mainchara.fun = true
    obj_mainchara.sprite_index = spr_mainchara_undynesit
...
if (con == 124)
{
    ...
    global.msg[1] = scr_gettext("obj_undynedate_inside_1001")
    instance_create(0, 0, obj_dialoguer)
    con = 125
}
...
if (con == 255)
{
    ...
    with (pot)
        instance_destroy()
    __background_set(13, 0, c_maroon)
...
{
    instance_destroy(obj_controlspear)
    global.faceemotion = 0
    instance_create(0, 0, obj_musfadeout)
    con = 301
    alarm[4] = 15
...
if (con == 360)
{
    undyne.sprite_index = spr_undyne_d_sit_dynamic
    undyne.depth = (table.depth - 1)
    undyne.y += 15
    con = 361
    alarm[4] = 30
...
if (con == 365.1)
{
    ...
    {
        if (OBJ_WRITER.stringno == 0 || OBJ_WRITER.stringno == 6)
        {
            undyne.sprite_index = spr_undyne_d_sit_dynamic_mad
...
if (con == 367.2)
{
    if instance_exists(OBJ_WRITER)
    {
        ...
        {
            undyne.sprite_index = spr_undyne_d_sit_dynamic_sad
            undyne.image_speed = 0.2
        }
        else
        {
...
if (con == 376 && instance_exists(OBJ_WRITER) == 0)
{
