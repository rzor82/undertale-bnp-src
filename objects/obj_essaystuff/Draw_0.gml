...
if (con == 1)
{
    ...
    if (string_pos("gaster", mystring_l) > 0 || string_pos("がすたー", mystring_l) > 0)
        gastered = 1
    if (gastered == 1)
    {
        global.msg[0] = "Nothing to say^1,&darling^1?/"
        global.msg[1] = "Can't say that&I blame you./%%"
    }
    done = 0
    if (swear == 1 && done == 0)
    {
...
if (con == 4 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    {
        ossafe_ini_open("undertale.ini")
        ini_write_real("MTT", "EssayScore", obj_ratingsmaster.essay)
        var charname = string_letters(global.charname)
        if (string_length(charname) > 6)
            charname = string_copy(charname, 0, 6)
        if (charname == "")
            charname = "ANONYMOUS"
        else
        {
            var low = string_lower(charname)
            if (low == "con" || low == "aux" || low == "nul" || low == "prn" || low == "com")
                charname = "INVALID"
        }
        var desktop = ((environment_get_variable("HOMEDRIVE") + environment_get_variable("HOMEPATH")) + "\\Desktop\\")
        var savedir = (((environment_get_variable("LOCALAPPDATA") + "\\") + game_project_name) + "\\")
        var filename = (("Essay_" + charname) + "_0.txt")
        var num = 0
        if file_exists("FileManager.dll")
        {
            while (ossafe_file_exists(filename) || file_exists_fmns((desktop + filename)))
            {
                num++
                filename = (((("Essay_" + charname) + "_") + string(num)) + ".txt")
            }
            filename = get_save_filename_ext("*.txt", filename, desktop, "WHERE TO SAVE YOUR ESSAY, FABULOUS?")
        }
        else
        {
            while ossafe_file_exists(filename)
            {
                num++
                filename = (((("Essay_" + charname) + "_") + string(num)) + ".txt")
            }
        ...
        if result
        {
            ...
            file_text_write_string(file, string_replace(mystring, "\n", "#"))
            file_text_close(file)
            ini_write_string("MTT", "Essay", filename)
        }
        else
            global.msg[0] = "* Mettaton failed to save &  your essay for future use^3.&* :("
        if (global.debug == true)
        {
            show_debug_message(((("Saved to " + filename) + ": ") + string(result)))
            if result
                global.msg[0] = ("\\G" + global.msg[0])
            else
                global.msg[0] = ("\\R" + global.msg[0])
        }
        ossafe_ini_close()
    }
    global.mnfight = 3
