...
if instance_exists(obj_toroverworld3)
{
    ...
    if (conversation == 4.5 && instance_exists(OBJ_WRITER) == 0 && toriel.path_position == 1)
    {
        with (toriel)
            path_end()
        global.hp = global.maxhp
        if (global.hp < global.maxhp || global.hp < 4)
            snd_play(snd_power)
        conversation = 5
    }
    if (conversation == 5 && instance_exists(OBJ_WRITER) == 0 && toriel.path_position == 1)
    {
        with (toriel)
            path_end()
        conversation = 6
        global.msg[0] = scr_gettext("obj_torieltrigger5_190")
        global.msg[1] = scr_gettext("obj_torieltrigger5_191")
        global.msg[2] = scr_gettext("obj_torieltrigger5_192")
        global.msg[3] = scr_gettext("obj_torieltrigger5_193")
        global.msg[4] = scr_gettext("obj_torieltrigger5_194")
        instance_create(0, 0, obj_dialoguer)
    }
}
if instance_exists(obj_toroverworld3)
...
if (fademusicout == 3 && instance_exists(OBJ_WRITER) == 0)
{
    torvolume -= 0.04
    if (volume < 0.9)
        volume += 0.06
