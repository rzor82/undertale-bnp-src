if (con > 0)
{
    ...
    {
        scr_colorcycle()
        if (instance_exists(obj_soulvision) && obj_soulvision.darken)
            alpha = (1 - obj_soulvision.alpha)
        else
            alpha = 1
    }
}
...
if (con == 17 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    h[3] = instance_create(150, 220, obj_heartcontainer)
    ...
    with (h[3])
    {
        target = 1
        with (obj_mainchara)
            other.y = ((y + sprite_height) + other.sprite_height)
    }
    con = 18
    alarm[4] = 150
}
