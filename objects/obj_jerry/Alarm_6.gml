if (ditch == 0)
{
    ...
    if (scr_monstersum() == 1)
    {
        ...
        if (mycommand > 20)
        {
            ...
            if (global.currentroom >= room_ruins1 && global.currentroom <= room_ruins19)
                global.msg[0] = "Gee, it sure is&BORING around here."
            if (global.currentroom >= room_water1 && global.currentroom <= room_water_undynefinal)
                global.msg[0] = "I got all WET!&Does ANYONE care?"
            if (global.currentroom >= room_fire1 && global.currentroom <= room_fire_precore)
                global.msg[0] = "Man, it's HOT.&Does ANYONE care?"
            if (global.currentroom >= room_fire_core1 && global.currentroom <= room_fire_finalelevator)
                global.msg[0] = "Ummm...&Am I supposed to&be here?"
        }
        if (mycommand > 40)
