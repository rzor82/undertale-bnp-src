...
if (con == 97.5 && instance_exists(OBJ_WRITER) == 0)
{
    snd_play(snd_phone)
    shblcon = instance_create(510, 200, obj_blconsm)
    shblcon.sprite_index = spr_shockblcon2
    with (shblcon)
        depth -= 4
    global.typer = 22
    global.msg[0] = "\\XMETTATON^1, YOU'RE&MY FAVORITE&SEXY RECTANGLE!/%%"
    instance_create(410, 60, OBJ_WRITER)
    OBJ_WRITER.depth = (shblcon.depth - 1)
    con = 98
}
if (con == 98 && instance_exists(OBJ_WRITER) == 0)
{
    snd_play(snd_phone)
    shblcon = instance_create(540, 200, obj_blconsm)
    shblcon.sprite_index = spr_shockblcon2
    with (shblcon)
        depth -= 8
    global.typer = 92
    global.msg[0] = scr_gettext("obj_mettatonex_928")
    instance_create(440, 60, OBJ_WRITER)
    OBJ_WRITER.depth = (shblcon.depth - 1)
    con = 99
}
