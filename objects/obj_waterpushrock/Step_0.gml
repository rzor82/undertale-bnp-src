...
if (active == true && global.interact != 5 && global.interact != 1)
{
    if instance_exists(obj_waterpushrockgen)
    {
        if (obj_waterpushrockgen.colliding == 0)
        {
            ...
            if collision_rectangle(bbox_left, (bbox_bottom + 1), bbox_right, ((bbox_bottom + vspeed) + 1), obj_mkid_actor, 0, 1)
            {
                obj_mkid_actor.y += (vspeed - 0.5)
                obj_mkid_actor.x = round(obj_mkid_actor.x)
                obj_mkid_actor.y = round(obj_mkid_actor.y)
                obj_waterpushrockgen.colliding = 1
            }
        }
    }
...
if (y > bottomy)
{
    ...
    if (room == room_water7 || room == room_water_firepuzzle)
        d.alarm[0] = 2
    instance_destroy()
}
if (y > 350)
