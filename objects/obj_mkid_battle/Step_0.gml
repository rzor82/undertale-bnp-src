...
if (global.hurtanim[myself] == 5)
{
    ...
    instance_create(((250 + (sprite_get_width(spr_undyneb_fatal) / 2)) - 48), (y - 24), obj_dmgwriter)
    with (obj_dmgwriter)
        alarm[2] = 30
    global.myfight = 0
    global.mnfight = 1
...
if (con == 18)
{
    ...
    undyne = instance_create(250, 20, obj_mkid_b_body)
...
if (con == 21.2)
{
    ...
    blcon = scr_blcon((mypart1.x - 250), mypart1.y, 1)
    con = 23
}
if (con == 23 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    scr_blcon_x((undyne.x + 150), undyne.y)
    con = 24
}
if (con == 24 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    scr_blcon((mypart1.x - 250), mypart1.y, 1)
    con = 25
}
...
if (con == 32)
    mypart1.image_alpha -= 0.03
if (con == 199)
    mypart1.image_alpha -= 0.08
if (con == 33)
{
    if (undyne.x <= 210)
    {
...
if (con == 44)
{
    with (wht)
        instance_destroy()
    caster_free(risesfx)
    siner.dimension_override = 1
    undyne.sprite_index = spr_undynex_example
    siner.height = (undyne.sprite_height * 2)
    siner.width = (undyne.sprite_width * 2)
    siner.image_alpha = 1
    siner.idealww *= 1.5
    siner.ww = siner.idealww
    undyne.y = -40
    siner.y = -40
    siner.yy = 0
    con = 45
