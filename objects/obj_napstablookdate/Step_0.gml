...
if (con == 40)
{
    global.screen_border_alpha = max(0, (alarm[5] / 300))
    buffer -= 1
    global.interact = 1
    if (buffer < 1)
        global.interact = 0
...
{
    waver.image_alpha -= 0.005
    global.screen_border_alpha += 0.005
    if (waver.image_alpha <= 0)
    {
        waver.image_alpha = 0
        if (!obj_napstablookdate_music.songplaying)
        {
            wavein = 0
            with (waver)
                instance_destroy()
...
if (wavein > 0 && global.screen_border_alpha < 1)
    global.screen_border_alpha += 0.05
