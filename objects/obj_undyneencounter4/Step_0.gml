...
if (con == 13)
{
    ...
    spr_prev_xoff = sprite_get_xoffset(spr)
    spr_prev_yoff = sprite_get_yoffset(spr)
    sprite_set_offset(spr, (sprite_get_width(spr) / 2), (sprite_get_height(spr) / 2))
    arm = scr_marker(obj_mainchara.x, obj_mainchara.y, spr)
    arm.x += (sprite_get_width(spr) / 2)
    arm.y += (sprite_get_height(spr) / 2)
    if (behind == 1)
        arm.depth = (obj_mainchara.depth + 5)
    con = 14
    alarm[4] = 110
}
if (con == 14 || con == 15)
...
if (con == 15)
{
    ...
    with (arm)
        instance_destroy()
    sprite_set_offset(spr, spr_prev_xoff, spr_prev_yoff)
    alarm[4] = 3
    con = 16
}
if (con == 17)
