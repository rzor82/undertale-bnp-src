...
if (q == 5)
{
    ...
    correct = 0
    event_user(1)
    with (instance_create(0, 0, obj_flyjar))
    {
        depth = (obj_battlecontroller.depth - 2)
        x = ((global.idealborder[0] + (c_borderwidth(1) / 5)) - (sprite_width / 2))
        y = ((global.idealborder[2] + (c_borderheight(1) / 2)) - (sprite_height / 2))
    }
}
if (q == 6)
{
    ...
    correct = 3
    event_user(1)
    with (instance_create(250, 80, obj_zoomaton))
    {
        depth = (other.tv.depth - 1)
        image_alpha = 0
        visible = false
    }
}
