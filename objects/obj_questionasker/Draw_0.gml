...
if (global.debug == true)
{
    if (phase == -1)
    {
        q += (keyboard_check_pressed(vk_add) - keyboard_check_pressed(vk_subtract))
        if (q < 0)
            q = 0
        if (q > 10)
            q = 10
...
if (q == 9)
{
    if (alarm[4] < 140 && con == 0)
    {
        ...
        scr_blcon((x - 240), (y - 10), 1)
        with (writer)
            writingy -= 20
        con = 1
    }
    else if (con == 4 && x > xstart)
        x -= 5
    else if (con == 0 && x < (xstart + 60))
        x += 5
}
if (con == 1)
{
...
if (variable_instance_exists(id, "blcon") && variable_instance_exists(id, "writer"))
{
    with (writer)
        depth = (other.tv.depth - 3)
    with (blcon)
        depth = (other.tv.depth - 2)
}
