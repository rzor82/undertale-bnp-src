...
var yoff = sin((siner / 6))
var xoff = cos((siner / 3))
draw_set_color(0x421F2A)
draw_set_alpha(image_alpha)
ossafe_fill_rectangle((x - 40), ((y + 20) + rely), (x + 42), ((y + 46) + rely))
draw_set_alpha(1)
draw_sprite_ext(spr_asrielfeet, 0, (x + (yoff * 2)), (((y + 168) - 112) + (rely * 1.2)), (2 + stetch), 2, torsorot, image_blend, image_alpha)
draw_sprite_ext(spr_torsoball, 0, (x + yoff), ((y + 48) + rely), (2 + stetch), 2, 0, image_blend, image_alpha)
draw_sprite_ext(spr_asrieltorso, 0, x, ((y + 48) + (rely * 1.2)), (2 + stetch), 2, torsorot, image_blend, image_alpha)
if (specialarm == 0)
{
    if (shrug == 0)
    {
...
draw_sprite_ext(spr_asrielcollar, 0, x, ((y + 22) + (rely * 1.2)), (2 + stetch), 2, 0, image_blend, image_alpha)
draw_sprite_ext(spr_asriellocket, 0, (x + 2), ((y + 32) + (rely * 1.2)), (2 + stetch), 2, 0, image_blend, image_alpha)
if (shrug == 0)
...
if (gonercon > 0)
{
    ...
    if (gonercon == 12)
    {
        if (instance_exists(obj_roundedge) && obj_roundedge.rotspeed > -2)
        {
            if (obj_roundedge.rotspeed > 0)
                obj_roundedge.rotspeed -= 0.3
            else
                obj_roundedge.rotspeed -= 0.1
        }
    }
    if (gonercon == 13)
    {
...
if (specialnormal == 0 && global.mnfight == 0 && global.myfight == 0)
{
    if (global.bmenuno == 1 || global.bmenuno == 2)
    {
        ...
        for (i = 0; i < len; i += 1)
        {
            ...
            var finalx = (textx + (sin(((siner + i) / 5)) * 8))
            var finaly = (270 + (cos(((siner + i) / 5)) * 4))
            xoff = 1
            yoff = 1
            draw_set_color(c_black)
            draw_text((finalx - xoff), finaly, letter)
            draw_text(finalx, (finaly - yoff), letter)
            draw_text(finalx, (finaly + yoff), letter)
            draw_text((finalx + xoff), finaly, letter)
            draw_text((finalx - xoff), (finaly - yoff), letter)
            draw_text((finalx + xoff), (finaly - yoff), letter)
            draw_text((finalx - xoff), (finaly + yoff), letter)
            draw_text((finalx + xoff), (finaly + yoff), letter)
            tcolor = make_color_hsv(((siner * 8) + (i * 8)), 140, 255)
            draw_set_color(tcolor)
            draw_text_transformed(finalx, finaly, string_hash_to_newline(letter), 1, 1, 0)
            if (global.language == "ja")
            {
                var ch = ord(letter)
                if (ch == 32 || ch >= 65377)
