if (audio_sound_get_pitch(global.batmusic) > 1.35 || berserk)
    caster_set_pitch(global.batmusic, 1.35)
        ...
        if (global.heard == 0)
        {
            ...
            if (whatiheard == 3)
            {
                ...
                if (plead_e == 1)
                {
                    ...
                    if (audio_sound_get_pitch(global.batmusic) > 1)
                        caster_set_pitch(global.batmusic, (audio_sound_get_pitch(global.batmusic) - 0.05))
                    if (audio_sound_get_pitch(global.batmusic) < 1)
...
if (con == 71 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    ddd = instance_create(melter.x, (melter.y + 5), obj_vaporized)
    ddd.image_speed = 0
    ddd.image_index = 1
    global.kills += 1
    global.xp += 500
...
if (died == 1)
    global.mercy = 1
else if (obj_heart.sprite_index == spr_heart)
    global.mercy = 0
else if (obj_heart.sprite_index == spr_heartgreen)
    global.mercy = 1
